...
export function useVapi(book: IBook) {
    ...
    const [maxDurationSeconds, setMaxDurationSeconds] = useState(5 * 60); // default: 5 min (Free plan)
    ...
    const maxDurationRef = useRef(5 * 60);

    // Sync refs with state for use in timers/intervals
    useEffect(() => {
    ...
    useEffect(() => {
        const handlers = {
            'call-start': () => {
                ...
                setStatus('listening'); // Initial status while waiting for first interaction or first message
                setCurrentMessage('');
                    ...
                    if (startTimeRef.current) {
                        ...
                        if (newDuration >= maxDurationRef.current) {
                            ...
                            setLimitError(
                                ...
                                )} minutes) reached. Redirecting...`,
                            );
                            // Set a short delay and then redirect
                            setTimeout(() => {
            ...
            error: (error: any) => {
                ...
                if (errorMessage.includes('timeout') || errorMessage.includes('silence')) {
                    ...
                    setLimitError(`Session ended unexpectedly: ${error.message || 'Unknown error'}. Click the mic to start again.`);
                }
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            // Use server-returned maxDurationMinutes to display the correct timer limit
            if (result.maxDurationMinutes) {
                setMaxDurationSeconds(result.maxDurationMinutes * 60);
            }

            // Validate Assistant ID before starting
            if (!ASSISTANT_ID) {
                console.error("Vapi Assistant ID is missing in environment variables!");
                throw new Error('Vapi Assistant ID is missing in environment variables.');
            }

            const firstMessage = `Hey, good to meet you. Quick question before we dive in - have you actually read ${book.title} yet, or are we starting fresh?`;

            console.log("Starting Vapi call with full configuration...");
            ...
            console.error('Failed to start call detail:', err);
            // Log full error for user to see
            const msg = err.message || (typeof err === 'string' ? err : JSON.stringify(err));
            setStatus('idle');
            setLimitError(`Failed to start voice session: ${msg}. Please try again.`);
        }
